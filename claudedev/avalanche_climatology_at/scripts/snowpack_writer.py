...
def write_sno(
    ...
    init_date = pd_date - timedelta(days=1)
    profile_timestamp = f"{init_date.isoformat()}T00:00"
    ...
    content = textwrap.dedent(f"""\
        ...
        fields = timestamp Layer_Thick  T  Ice  Water  Salinity  Dendricity  Sphericity  Coordination_Index  Marker  Hard
        [DATA]
    """)
...
def write_snowpack_ini(
    ...
    """
    profile_step = int(sp_cfg.get("profile_output_step", 24))
    write_stability = str(sp_cfg.get("stability", True)).upper()

    output_path.parent.mkdir(parents=True, exist_ok=True)

    ini_content = textwrap.dedent(f"""\
        ; SNOWPACK configuration for flat-field avalanche climatology simulation
        ; Station: {station_id}
        ...
        WATER_LAYER         = FALSE
        MASS_BALANCE        = FALSE
        FORCE_SOFTWARE_RADIATION = FALSE
        SW_MODE             = INCOMING
        ALLOW_SURFACE_PONDING = FALSE
        ...
        METEO_STEP_LENGTH       = {meteo_step}
        SLOPE_ANGLE             = {slope_angle:.1f}
        SLOPE_ASP               = {slope_aspect:.1f}
